...
class SyncView extends Component {
	...
	async smartSync(smart) {
		...
		.then(response => response.json())
		.then(response => {
			if(response && response.error && response.error == 'token_expired') {
		...
		return (
			<ScrollView style={styles.container}>
				...
				<List containerStyle={{ marginTop: 5 }}>
					<ListItem titleStyle={styles.list}
						hideChevron={true}
						title="Atualização Agenda"
						leftIcon={{ name: this.state.isUpdateSchedules ? 'check' : 'cached', 'color':  this.state.isUpdateSchedules ? '#32CD32' : ''}}
					...
					<ListItem
						...
						leftIcon={{ name: this.state.isUpdateProductions ? 'check' : 'cached' , 'color':  this.state.isUpdateProductions ? '#32CD32' : '' }} />
					<ListItem
						titleStyle={styles.list}
						hideChevron={true}
						title="Atualização Questionário"
						leftIcon={{ name: this.state.isUpdateAnswersPlace ? 'check' : 'cached' , 'color':  this.state.isUpdateAnswersPlace ? '#32CD32' : '' }} />
					<ListItem
						titleStyle={styles.list}
						hideChevron={true}
						title="Safras"
						leftIcon={{ name: this.state.isCropsInSync ? 'check' : 'cached' , 'color':  this.state.isCropsInSync ? '#32CD32' : '' }} />
					<ListItem
						titleStyle={styles.list}
						hideChevron={true}
						title="Cultivares"
						leftIcon={{ name: this.state.isCultivarsInSync ? 'check' : 'cached' , 'color':  this.state.isCultivarsInSync ? '#32CD32' : '' }} />
					<ListItem
						titleStyle={styles.list}
						hideChevron={true}
						title="Marcas de Sementes"
						leftIcon={{ name: this.state.isSeedBrandsInSync ? 'check' : 'cached' , 'color':  this.state.isSeedBrandsInSync ? '#32CD32' : '' }} />
					<ListItem
						titleStyle={styles.list}
						hideChevron={true}
						title="Fazendas"
						leftIcon={{ name: this.state.isPlacesInSync ? 'check' : 'cached' , 'color':  this.state.isPlacesInSync ? '#32CD32' : '' }} />
					<ListItem
						titleStyle={styles.list}
						hideChevron={true}
						title="Produções"
						leftIcon={{ name: this.state.isProductionsInSync ? 'check' : 'cached' , 'color':  this.state.isProductionsInSync ? '#32CD32' : '' }} />
					<ListItem
						titleStyle={styles.list}
						hideChevron={true}
						title="Agenda"
						leftIcon={{ name: this.state.isSchedulesInSync ? 'check' : 'cached' , 'color':  this.state.isSchedulesInSync ? '#32CD32' : '' }} />
					<ListItem
						titleStyle={styles.list}
						hideChevron={true}
						title="Perguntas"
						leftIcon={{ name: this.state.isQuestionsInSync ? 'check' : 'cached' , 'color':  this.state.isQuestionsInSync ? '#32CD32' : '' }} />
					<ListItem
						titleStyle={styles.list}
						hideChevron={true}
						title="Respostas"
						leftIcon={{ name: this.state.isAnswersInSync ? 'check' : 'cached' , 'color':  this.state.isAnswersInSync ? '#32CD32' : '' }} />
					<ListItem
						titleStyle={styles.list}
						hideChevron={true}
						title="Recursos"
						leftIcon={{ name: this.state.isLastSyncInSync ? 'check' : 'cached' , 'color':  this.state.isLastSyncInSync ? '#32CD32' : '' }} />
				</List>
			</ScrollView>
		)
	}
